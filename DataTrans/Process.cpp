...
STDMETHODIMP CProcess::MultiImportToSQL(IDispatch* pIDisp, IdeSQL* pSQL, VARIANT_BOOL* pRetVal)
{
	...
	if (pIDisp == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::MultiImportToSQL() -- Either input source object or deSQL object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (pFixed == NULL && pDelim == NULL ){
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::MultiImportToSQL() -- Input source object is neither deFixed or deDelim object. Processing can not continue.");
		return S_OK;
	}
	...
	if (Utilities::GetDirectoryPath(szFileName, szDirPath) != 0 )  {  

		//filename can contain full path or partial path name
		_bstr_t bstrDirPath(szDirPath);
		if (Utilities::IsFullPath(szFileName ) )
			bstrDir = bstrDirPath;
		else 
			bstrDir = Utilities::ConstructFullPath(bstrDir, bstrDirPath);
		if (pFixed != NULL)
	...
	iStrLen =  SysStringLen(bstrDir);
	WideCharToMultiByte(CP_ACP, 0, bstrDir, iStrLen, szDir, iStrLen+1, NULL, NULL);
	szDir[iStrLen]= '\0';
	...
	if ( Utilities::ParseOutFileName(szFileName, szFileBuffer) > 0) {
		_bstr_t bstrFileBuffer(szFileBuffer);
		bstrFile = bstrFileBuffer;
	}

	TCHAR szFile[BUFF_MED]; memset(szFile, 0, BUFF_MED);
	iStrLen =  SysStringLen(bstrFile);
	WideCharToMultiByte(CP_ACP, 0, bstrFile, iStrLen, szFile, iStrLen+1, NULL, NULL);
	szFile[iStrLen]= '\0';

	if ((hFind = FindFirstFile(szFile, &wfd)) != INVALID_HANDLE_VALUE) {

		BOOL bResult = FALSE;
		iFileCnt++;
		_bstr_t bstrTemp(wfd.cFileName);
		listFileName.push_back(bstrTemp);
		
		while ( FindNextFile(hFind, &wfd) == TRUE) {
			_bstr_t bstrTemp1(wfd.cFileName);
			listFileName.push_back(bstrTemp1);
			iFileCnt++;
		}
		FindClose(hFind);
	}
...
//	_bstr_t bstrErrors = L"";
	...
	SysFreeString(bTemp);
	TCHAR szOwner[BUFF_MED]; memset(szOwner, 0, BUFF_MED);
	iStrLen =  SysStringLen(bstrOwner);
	WideCharToMultiByte(CP_ACP, 0, bstrOwner, iStrLen, szOwner, iStrLen+1, NULL, NULL);
	szOwner[iStrLen]= '\0';
	...
	iStrLen =  SysStringLen(bstrTable);
	...
	TCHAR szSrceTable[BUFF_MED]; memset(szSrceTable, 0, BUFF_MED);
	iStrLen =  SysStringLen(bstrSrceTable);
	WideCharToMultiByte(CP_ACP, 0, bstrSrceTable, iStrLen, szSrceTable, iStrLen+1, NULL, NULL);
	szSrceTable[iStrLen]= '\0';
		...
		if ( SUCCEEDED(objADO.OpenTrustedConnection(bstrServer, bstrDatabase)) ) 
		{
			...
			if ( objADO.TableExists(bstrServer, bstrDatabase, bstrSrceTable) ) 
			{
				if (m_nAction == vAPPEND) 
				{
					...
					_stprintf(szQuery, _T("SELECT MAX(SourceID) from %s"), szSrceTable);
					ADODB::_RecordsetPtr spRecordset = NULL;
					_bstr_t bstrQuery(szQuery);
					if (SUCCEEDED(objADO.GetRecordSet((IDispatch**) &spRecordset, bstrQuery)) ) 
					{
						if ( spRecordset != NULL) 
						{
				...
				{
					memset(szQuery, 0, MAX_QUERY_LEN);
					_stprintf(szQuery, _T("DROP TABLE %s"), szSrceTable);
			...
			if (m_nAction == vAPPEND || m_nAction == vOVERWRITE) //won't be there on vCREATE or vOVERWRITE
			{
				...
				else 
				{
					if (m_nAction == vAPPEND)
					{	//add the column  
						_stprintf(szQuery, pzAlterSQL, szTable);
						if (!SUCCEEDED(objADO.ExecuteQuery(szQuery))) 
						{
							Utilities::TransferErrors(objADO.m_listErrors, m_listErrors);
							m_bAddSource = VARIANT_FALSE; //turn this off, something is amiss...
				...
				{
					memset(szQuery, 0, MAX_QUERY_LEN);
					_stprintf(szQuery, pzUpdateSQL, szOwner, szTable, 0);
					if (!SUCCEEDED(objADO.ExecuteQuery(szQuery)))
						Utilities::TransferErrors(objADO.m_listErrors, m_listErrors);
				}
			}
	...
	for (iterList = listFileName.begin(); iterList != listFileName.end(); iterList++)  {
		...
		_bstr_t bstrFileTemp = (_bstr_t)(*iterList);
		bTemp = SysAllocString(bstrFileTemp);
		if (pFixed != NULL)
			pFixed->put_FileName(bTemp);
		else
            pDelim->put_FileName(bTemp);
		SysFreeString(bTemp);
		TCHAR szFileTemp[BUFF_MED]; memset(szFileTemp, 0, BUFF_MED);
		int iStrLen =  SysStringLen(bstrFileTemp);
		WideCharToMultiByte(CP_ACP, 0, bstrFileTemp, iStrLen, szFileTemp, iStrLen+1, NULL, NULL);
		szFileTemp[iStrLen]= '\0';
...
//		bstrErrors += m_bstrErrMsgs;
		...
		else 
		{
			if (m_bAddSource ) 
			{
				if (bAddTable) 
				{
					...
					memset(szQuery, 0, MAX_QUERY_LEN);
					_stprintf(szQuery, "CREATE TABLE %s.[%s] (SourceID int, Source varchar(500) )", szOwner, szSrceTable );
					if (!SUCCEEDED(objADO.ExecuteQuery(szQuery))) {
						Utilities::TransferErrors(objADO.m_listErrors, m_listErrors);
						m_bAddSource = VARIANT_FALSE; //turn this off, something is amiss...
					}
					...
					memset(szQuery, 0, MAX_QUERY_LEN);
					_stprintf(szQuery, pzInsertSQL, szOwner, szSrceTable, 0, _T("missing") );
					if (!SUCCEEDED(objADO.ExecuteQuery(szQuery))) {
						Utilities::TransferErrors(objADO.m_listErrors, m_listErrors);
						m_bAddSource = VARIANT_FALSE; //turn this off, something is amiss...
					}
				...
				memset(szQuery, 0, MAX_QUERY_LEN);
				_stprintf(szQuery, pzInsertSQL, szOwner, szSrceTable, iSrceCnt, szFileTemp );
				if (!SUCCEEDED(objADO.ExecuteQuery(szQuery)))
						Utilities::TransferErrors(objADO.m_listErrors, m_listErrors);

				//update target table with source id 
				memset(szQuery, 0, MAX_QUERY_LEN);
				_stprintf(szQuery, pzUpdateSQL, szOwner, szTable,  iSrceCnt);
				if (!SUCCEEDED(objADO.ExecuteQuery(szQuery)))
						Utilities::TransferErrors(objADO.m_listErrors, m_listErrors);
			}
		}
	...
	if (iFileCnt == 0) {
		_stprintf(szError, _T("DTRAN ERROR: CProcess::MultiFixedToSQL() -- No files were found based on directory:  %s and file name: %s specified in input deFixed object."), szDir, szFile);
...
STDMETHODIMP CProcess::FixedToSQL(IdeFixed *pFixed, IdeSQL *pSQL,  VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (pFixed == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Either deFixed object or deSQL object is NULL.  Processing can not continue.");
			return S_OK;
	}
	...
	if (pFixedFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Format object in deFixed object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (pSQLFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Format object in deSQL object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (lFixedFldCnt == 0 && lSQLFldCnt == 0) 
	{
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Failure loading field collection via LoadFormat.");  
			BSTR bTemp;
			pFixed->get_ErrMsgs(&bTemp);
			_bstr_t bstrTemp = bTemp;
			SysFreeString(bTemp);
			_bstr_t bstrErrs = L"DTRAN ERROR: Errors from deFixed LoadFormat method: ";
			bstrErrs += bstrTemp;
			m_listErrors.push_back(bstrErrs);
		}

		pFixedFormat->get_FieldCount(&lFixedFldCnt);
		if (lFixedFldCnt == 0)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Field collection is empty.Processing can not continue.");
		else
			bLoadSQL = true;
	}
	else 
	...
	if (bLoadSQL) {
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Attempt to copy deDelim format into deSQL object failed.");
	...
	if (bSuccess) {
		...
		if (wcscmp(bstrFormatLoc, L"") == 0 ) {
			LPTSTR pzFormatFile =  Utilities::ConstructFileName("BCPFormat", "FMT");
			bstrFormatLoc  = A2W(pzFormatFile);
		...
		try {
			...
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Landed in catch statement during transfer.");
		}
...
//	bool bExportQuery = false;
	...
	if (pFixed == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Either deFixed object or deSQL object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	pFixed->get_Format(&pFixedFormat);
	if (pFixedFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Format object in deFixed object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	pSQL->get_Format(&pSQLFormat);
	if (pSQLFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Format object in deSQL object is NULL.  Processing can not continue.");
		return S_OK;
	}
...
//		bExportQuery = true;
	...
	if (lFixedFldCnt == 0 && lSQLFldCnt == 0) 
	{
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Failure loading field collection via LoadFormat.");  //for now....
			BSTR bTemp;
			pSQL->get_ErrMsgs(&bTemp);
			_bstr_t bstrTemp = bTemp;
			SysFreeString(bTemp);
			_bstr_t bstrErrs = L"DTRAN ERROR: Errors from deSQL LoadFormat method: ";
			bstrErrs += bstrTemp;
		}
		pSQLFormat->get_FieldCount(&lSQLFldCnt);
		if (lSQLFldCnt == 0)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Field collection is empty.Processing can not continue.");
		else
			bLoadFixed = true;
	}
	else 
	...
	if (bLoadSQL) {
		pSQLFormat->CopyFormat(pFixedFormat, &bSuccess);
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Attempt to copy deDelim format into deSQL object failed.");
			TransferErrors(&pSQLFormat);
		}
	}
...
//	_bstr_t bstrOrigOwner = bTemp;
	...
	_bstr_t bstrExtraField(sz_ExtraFieldName);
	if ( bSuccess ) {
	...
	if (bSuccess) 
	{
		try {
			...
			if (!objBCP.ErrorsOccurred() )	{
				...
				if (SUCCEEDED(hr) && !objBCP.ErrorsOccurred() ) {
					...
					if (Utilities::ChangeFileExtension(objBCP.m_bFileLoc, _T("fmt"), szFormatName) > 0 )  {
						_bstr_t bstrFormatName(szFormatName);
						pSQLFormat->Write(bstrFormatName, vCUSTFORMAT, &bSuccess);
					}
					m_lRowsLoaded = objBCP.m_lRowsLoaded;
			...
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToFixed() -- Landed in catch statement during transfer.");
		}
...
STDMETHODIMP CProcess::SQLToDelim(IdeSQL *pSQL, IdeDelim *pDelim, VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (pDelim == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() --  Either deFixed object or deSQL object is NULL.  Processing can not continue.");
			return S_OK;
	}
	...
	if (pDelimFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() -- Format object in deDelim object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (lDelimFldCnt == 0 && lSQLFldCnt == 0) 
	{
		pSQL->LoadFormat(&bSuccess);   
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() -- Failure loading field collection via LoadFormat.");  
			BSTR bTemp;
			pSQL->get_ErrMsgs(&bTemp);
			_bstr_t bstrTemp = bTemp;
			SysFreeString(bTemp);
		...
		pSQLFormat->get_FieldCount(&lSQLFldCnt);
		if (lSQLFldCnt == 0)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() -- Field collection is empty.Processing can not continue.");
		else
			bLoadDelim = true;
	}
	else 
	...
	if (bLoadSQL) {
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() -- Attempt to copy deDelim format into deSQL object failed.");
			TransferErrors(&pSQLFormat);
		}
	}
	...
	if (bSuccess) 
	{
		...
		try {
			...
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() -- Landed in catch statement during transfer.");
		}
		if( objDTS.ErrorsOccurred() )  {
...
STDMETHODIMP CProcess::DelimToSQL(IdeDelim *pDelim, IdeSQL *pSQL, VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (pDelim == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() --  Either deFixed object or deSQL object is NULL.  Processing can not continue.");
			return S_OK;
	}
#ifdef FILE_DEBUG
	else {
	...
	pDelim->get_Format(&pDelimFormat);
	if (pDelimFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- Format object in deDelim object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (ptrLog != NULL){
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- Failure loading field collection via pDelim->LoadFormat.");  //for now....
			BSTR bTemp;
			pDelim->get_ErrMsgs(&bTemp);
			_bstr_t bstrTemp = bTemp;
			SysFreeString(bTemp);
			_bstr_t bstrErrs = L"DTRAN ERROR: Errors from deDelim LoadFormat method: ";
			bstrErrs += bstrTemp;
			m_listErrors.push_back(bstrErrs);
		}
		pDelimFormat->get_FieldCount(&lDelimFldCnt);
		if (lDelimFldCnt == 0)
				m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- Delim Field collection is empty.Processing can not continue.");
	}
	...
	if (ptrLog != NULL){
		...
		fflush(ptrLog);
			...
			pSQL->LoadFormat(&bSuccess);   
			if (!bSuccess) {
				m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- Failure loading field collection via pSQL->LoadFormat.");  //for now....
				BSTR bTemp;
				pSQL->get_ErrMsgs(&bTemp);
				_bstr_t bstrTemp = bTemp;
				SysFreeString(bTemp);
			...
			pSQLFormat->get_FieldCount(&lSQLFldCnt);
			if (lSQLFldCnt == 0)
				m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- SQL Field collection is empty.Processing can not continue.");
		}
		else {
			if (lDelimFldCnt != 0 ) {
#ifdef FILE_DEBUG
	if (ptrLog != NULL){
		...
		fflush(ptrLog);
				...
				pSQLFormat->CopyFormat(pDelimFormat, &bSuccess);
				if (!bSuccess) {
					m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- Attempt to copy deDelim format into deSQL object failed.");
					TransferErrors(&pSQLFormat);
				}
			}
		}
	...
	if (bSuccess) 
	{
		...
		try {
			...
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::DelimToSQL() -- Landed in catch statement during transfer.");
		}
		if( objDTS.ErrorsOccurred() )  {
			Utilities::TransferErrors(objDTS.m_listErrors, m_listErrors);
			bSuccess = VARIANT_FALSE;
...
STDMETHODIMP CProcess::XBaseToSQL(IdeXBase *pXBase, IdeSQL *pSQL, VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (pXBase == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Either deFixed object or deSQL object is NULL.  Processing can not continue.");
			return S_OK;
	}
	...
	if (pXBaseFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Format object in deFixed object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	pSQL->get_Format(&pSQLFormat);
	if (pSQLFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Format object in deSQL object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (lXBaseFldCnt == 0 ) {
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Failure loading field collection via pXBase->LoadFormat."); 
			BSTR bTemp;
			pXBase->get_ErrMsgs(&bTemp);
			_bstr_t bstrTemp = bTemp;
			SysFreeString(bTemp);
			_bstr_t bstrErrs = L"DTRAN ERROR: Errors from deXBase LoadFormat method: ";
			bstrErrs += bstrTemp;
			m_listErrors.push_back(bstrErrs);
		}
		pXBaseFormat->get_FieldCount(&lXBaseFldCnt);
		if (lXBaseFldCnt == 0 )
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- XBase Field collection is empty.Processing can not continue.");
	}
	...
	if (lSQLFldCnt == 0  )  {
		if (m_nAction == vAPPEND || m_nAction == vOVERWRITE) {
			pSQL->LoadFormat(&bSuccess);
			if (!bSuccess) {
				m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Failure loading field collection via pSQL->LoadFormat.");  
				BSTR bTemp;
				pSQL->get_ErrMsgs(&bTemp);
				_bstr_t bstrTemp = bTemp;
				SysFreeString(bTemp);
			...
			if (lXBaseFldCnt != 0) {
				...
				if (!bSuccess) {
					m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Attempt to copy XBase format into deSQL object failed.");
					TransferErrors(&pSQLFormat);
				}
			}
		}
...
/*      //again multi loading....
		if ( m_iLoadCnt == 0 || !SUCCEEDED(hr) || objDTS.ErrorsOccurred() ) {	//last file to load or cleanup if multiload is going to abort
			...
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::XBaseToSQL() -- Landed in catch statement during transfer.");
		}
...
STDMETHODIMP CProcess::SQLToXBase(IdeSQL *pSQL, IdeXBase *pXBase, VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (pXBase == NULL || pSQL == NULL )  {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Either deFixed object or deSQL object is NULL.  Processing can not continue.");
			return S_OK;
	}
	...
	pXBase->get_Format(&pXBaseFormat);
	if (pXBaseFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Format object in deFixed object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	pSQL->get_Format(&pSQLFormat);
	if (pSQLFormat == NULL) {
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Format object in deSQL object is NULL.  Processing can not continue.");
		return S_OK;
	}
	...
	if (lXBaseFldCnt == 0 && lSQLFldCnt == 0) 
	{
		pSQL->LoadFormat(&bSuccess);   
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Failure loading field collection via LoadFormat."); 
			BSTR bTemp;
			pSQL->get_ErrMsgs(&bTemp);
			_bstr_t bstrTemp = bTemp;
			SysFreeString(bTemp);
		...
		pSQLFormat->get_FieldCount(&lSQLFldCnt);
		if (lSQLFldCnt == 0)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Field collection is empty.Processing can not continue.");
		else
			bLoadXBase = true;
	}
	else 
	...
	if (bLoadSQL) {
		pSQLFormat->CopyFormat(pXBaseFormat, &bSuccess);
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Attempt to copy deXBase format into deSQL object failed.");
		//	Utilities::TransferErrors(pSQLFormat->, m_listErrors);
			TransferErrors(&pSQLFormat);
		}
	}
	...
	if (nXBaseType != vDBASE ) {
		bSuccess = VARIANT_FALSE;
		m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- SQL exports to FoxPro tables are not supported.  Processing can not continue.");
	}
		...
		try {
			...
			if (!objDTS.ErrorsOccurred() )	{
				...
				if ( !m_bTableOK ) {
					...
					DBFFIELD FldsArray[NUMDBFFIELDS];
					IFieldInfo* pField = NULL;
					long lFldCntTemp = 0;
					...
					if (lFldCnt != 0 ) 
						...
						m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() No fields in deXBase Object."); //shouldn't get here but just in case...
				}
	...
	//	}
			...
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Landed in catch statement during transfer.");
		}
...
VARIANT_BOOL CProcess::ReconcileFieldColl(IFormat** pSrceFormat, IFormat** pTrgtFormat)
{
	...
	if (lSrceFldCnt == 0 && lTrgtFldCnt == 0) 
	{
		...
		if (!bSuccess)
			m_listErrors.push_back(L"DTRAN ERROR: CProcss::ReconcileFieldColl -- Failure loading field collection via LoadFormat.");  //for now....
		(*pSrceFormat)->get_FieldCount(&lSrceFldCnt);
		if (lSrceFldCnt == 0)
			m_listErrors.push_back(L"DTRAN ERROR: CProcss::ReconcileFieldColl -- Field collection is empty.Processing can not continue.");
		else
			bLoadSrce = true;
	}
	else 
	...
	if (bLoadTrgt) {
		...
		if (!bSuccess) {
			m_listErrors.push_back(L"DTRAN ERROR: CProcss::ReconcileFieldColl  -- Attempt to copy deDelim format into deSQL object failed.");
