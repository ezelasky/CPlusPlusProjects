		...
		if (pProcess != NULL)
		{
			...
			if (bXBaseObj == VARIANT_TRUE)
			{
				...
				if (pXBase != NULL) {
					...
					{
						m_listErrors.push_back( L"smaller catch....DTRAN ERROR:  CdeSQL::SQLToXBase in catch statement.");
					}
					pXBase->Release();
					pXBase = NULL;
				}
...
STDMETHODIMP CdeSQL::GetCreateTableSQL(BSTR *pRetVal)
{
	...
	while (pField != NULL) 
	{
		...
		case vDECIMAL: 
			bstrSQL += L" NUMERIC("; 
			bstrSQL += (_bstr_t) szPrec;
			bstrSQL += L", ";
			bstrSQL += (_bstr_t) szScale;
			bAddEndParan = true;
			break;
		case vFLOAT:
			bstrSQL += L" FLOAT";
...
STDMETHODIMP CdeSQL::Exists(VARIANT_BOOL *pRetVal)
{
	...
	try {
		...
		if ( SUCCEEDED(objADO.OpenTrustedConnection()) ) {
			...
			_bstr_t bQry = (_bstr_t) Utilities::ConstructInfoSchemaSQL(m_bstrTable, m_bstrOwner);
		...
		if ( SUCCEEDED(objADO.OpenTrustedConnection()) ) {
			...
			if (wcscmp(m_bstrExportQuery, L"") != 0 ) {
				...
				if (SUCCEEDED(objADO.CreateTableFromQuery(m_bstrExportQuery, pzTableName )))  {
					put_Table((_bstr_t) pzTableName);  // this action will delete any prior temporary tables..
					//m_bstrTable = (_bstr_t) pzTableName;  
					...
					if (iRecCount > 0)
					{
						...
						while (!spRecordset->adoEOF)
						{
								 ...
								 (vType.vt != VT_NULL && vType.vt != VT_EMPTY)       )  
							{
								...
								Utilities::ReplaceChar((TCHAR*) bTemp, szBuffer, TCHAR(' '), TCHAR('_'));
								...
								m_pFormatObj->AddFieldExt((_bstr_t) szBuffer, vLen.bstrVal, nType, lStartPos, lEndPos, lPrec, lScale, &bSuccess);
							}
	...
	try {
		...
		if ( SUCCEEDED(objADO.OpenTrustedConnection()) ) {
			...
			if (SUCCEEDED(hr) ) {
				...
				_stprintf(szError, _T("DTRAN ERROR: A connection could not be made on server: %s with database: %s"), (TCHAR*)m_bstrServer, (TCHAR*)m_bstrDatabase);
...
STDMETHODIMP CdeSQL::AddDatabase(BSTR DataBase, BSTR Path, BSTR DatSize, BSTR LogSize, BSTR DatGrowth, BSTR LogGrowth, BSTR DatMaxSize, BSTR LogMaxSize, VARIANT_BOOL *pRetVal)
{
	...
	try {
		...
		Utilities::RemoveLastSlash((TCHAR*)bstrPathIn, szPath);
		_bstr_t bstrTemp = Utilities::ConstructFullPath(szCurDir, szPath);
		memset(szPath, 0, MAX_FILE_LEN);
		Utilities::RemoveLastSlash((TCHAR*)bstrTemp, szPath);
		...
		if ( m_listErrors.size() == 0 ) {
			...
			if ( SUCCEEDED(objADO.OpenTrustedConnection()) ) {
				...
				_stprintf(szQuery, "CREATE DATABASE %s ON ( NAME = %s_dat, FILENAME = '%s\\%sdat.mdf', SIZE = %s, MAXSIZE = %s, FILEGROWTH = %s ) LOG ON ( NAME = '%s_log', FILENAME = '%s\\%slog.ldf', SIZE = %s, MAXSIZE = %s, FILEGROWTH = %s )", 
							(TCHAR*)bstrDataBase, (TCHAR*)bstrDataBase, szPath1, (TCHAR*) bstrDataBase, (TCHAR*) bstrDatSize, (TCHAR*) bstrDatMaxSize, (TCHAR*) bstrDatGrowth,
												  (TCHAR*)bstrDataBase, szPath1, (TCHAR*) bstrDataBase, (TCHAR*) bstrLogSize, (TCHAR*) bstrLogMaxSize, (TCHAR*) bstrLogGrowth);
...
*/
				...
				_stprintf(szError, _T("DTRAN ERROR: A connection could not be made on server: %s with the master database to create data base: %s"), (TCHAR*)m_bstrServer, (TCHAR*)bstrDataBase);
				m_listErrors.push_back(szError);
			}
		}
		else
	...
	try { 
		...
		if ( m_listErrors.size() == 0 ) {
			...
			if ( SUCCEEDED(objADO.OpenTrustedConnection()) ) {
				...
				ADODB::_RecordsetPtr spRecordset = NULL;
				_stprintf(szQuery, "select * from master..sysdatabases where name = '%s'", (TCHAR*) bstrDataBase );
			
				hr = objADO.GetRecordSet((IDispatch**) &spRecordset, (_bstr_t) szQuery);
				if (SUCCEEDED(hr) ) {
				...
				_stprintf(szError, _T("DTRAN ERROR: A connection could not be made on server: %s with the master data base to test data base: %s existance."), (TCHAR*)bstrServer, (TCHAR*)bstrDataBase);
				m_listErrors.push_back(szError);
			}
		}
	}
