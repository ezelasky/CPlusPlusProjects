...
//		bstrErrors += m_bstrErrMsgs;
	...
	if (iFileCnt == 0) {
		_stprintf(szError, _T("DTRAN ERROR: CProcess::MultiFixedToSQL() -- No files were found based on directory:  %s and file name: %s specified in input deFixed object."), (TCHAR*) bstrDir, (TCHAR*) bstrFile);
...
STDMETHODIMP CProcess::FixedToSQL(IdeFixed *pFixed, IdeSQL *pSQL,  VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (lFixedFldCnt == 0 && lSQLFldCnt == 0) 
	{
		...
		if (!bSuccess)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::FixedToSQL() -- Failure loading field collection via LoadFormat.");  //for now....
		pFixedFormat->get_FieldCount(&lFixedFldCnt);
		if (lFixedFldCnt == 0)
...
STDMETHODIMP CProcess::SQLToDelim(IdeSQL *pSQL, IdeDelim *pDelim, VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (lDelimFldCnt == 0 && lSQLFldCnt == 0) 
	{
		pSQL->LoadFormat(&bSuccess);   
		if (!bSuccess)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToDelim() -- Failure loading field collection via LoadFormat.");  //for now....
		pSQLFormat->get_FieldCount(&lSQLFldCnt);
		if (lSQLFldCnt == 0)
...
STDMETHODIMP CProcess::SQLToXBase(IdeSQL *pSQL, IdeXBase *pXBase, VARIANT_BOOL *pRetVal)
{
	...
	*pRetVal = bSuccess;
	...
	if (lXBaseFldCnt == 0 && lSQLFldCnt == 0) 
	{
		pSQL->LoadFormat(&bSuccess);   
		if (!bSuccess)
			m_listErrors.push_back(L"DTRAN ERROR: CProcess::SQLToXBase() -- Failure loading field collection via LoadFormat.");  //for now....
		pSQLFormat->get_FieldCount(&lSQLFldCnt);
		if (lSQLFldCnt == 0)
	...
	if (bSuccess) {
		...
		if (!objDTS.ErrorsOccurred() )	{
			...
			if ( !m_bTableOK ) {
				...
				DBFFIELD FldsArray[100];
				long lFldCnt = 0;

				char szErr[MAX_ERR_LEN];
				memset(szErr, 0, MAX_ERR_LEN);
				
				m_listErrors.push_back(L"start building field array");
				...
				while (pField != NULL) {
					...
					USES_CONVERSION;
					LPTSTR lpTemp = W2A(bTemp);
					//wcstombs(FldsArray[lFldCnt].Name, bTemp, DBFFIELDLEN-1);
					SysFreeString(bTemp);
                    //FldsArray[lFldCnt].Name[strlen(FldsArray[lFldCnt].Name)+1] = '\0';
					strncpy(FldsArray[lFldCnt].Name, lpTemp, DBFFIELDLEN-1);
					int iTemp = strlen(FldsArray[lFldCnt].Name);
					...
					FldsArray[lFldCnt].Name[strlen(FldsArray[lFldCnt].Name)+1] = '\0';
					_stprintf(szErr, _T("Field Name: %s"), FldsArray[lFldCnt].Name);
					_bstr_t b1(szErr);
					m_listErrors.push_back(b1);

					memset(szErr, 0, MAX_ERR_LEN);
					_stprintf(szErr, _T("Field Length: %d"), FldsArray[lFldCnt].Length);
					m_listErrors.push_back(A2W(szErr));

					char tempType[2];
					memset(tempType, 0, 2);
					tempType[0] = FldsArray[lFldCnt].Type;
					tempType[1] = '\0';
					_stprintf(szErr, _T("Field Type: %s"), tempType);
					m_listErrors.push_back(A2W(szErr));
					pField->Release();
				...
				if (lFldCnt != 0 ) {
					m_listErrors.push_back(L"Before ConfrimDBFTable");				
					hr = objDTS.ConfirmDBFTable((ACTION)m_nAction, FldsArray, lFldCntTemp);
					m_listErrors.push_back(L"After ConfrimDBFTable");	
				}
				else
		...
		//		if (FldsArray) delete [] FldsArray;
			...
			if ( objDTS.ErrorsOccurred() )
				m_listErrors.push_back(L"******************** errors occurred in DTS operations");
			else
				m_listErrors.push_back(L"******************** errors DID NOT occurr in DTS operations");
